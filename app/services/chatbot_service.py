...
class RAGPipeline:
    ...
    # --- 동기 버전 헬퍼 함수들 ---
    def _filter_question_sync(self, question):
        prompt = self.classify_prompt.format(question=question)
        try:
            completion = self.sync_llm_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": prompt}],
                temperature=0
            )
            ...
            print(f"[SYNC] _filter_question_sync exception: {e}")
            return False

    def _embed_question_sync(self, question):
        try:
            return self.embedder.embed(question)
        except Exception as e:
            print(f"[SYNC] _embed_question_sync exception: {e}")
            raise

    def _search_similar_questions_sync(self, query_vec, top_k):
        try:
            return self.milvus.search(query_vec, top_k)
        except Exception as e:
            print(f"[SYNC] _search_similar_questions_sync exception: {e}")
            raise

    def _build_context_sync(self, results):
        try:
            return "\n\n".join([f"Q: {r['question']}\nA: {r['answer']}" for r in results])
        except Exception as e:
            print(f"[SYNC] _build_context_sync exception: {e}")
            raise

    def _call_llm_sync(self, prompt):
        try:
            completion = self.sync_llm_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": prompt}],
            ...
            print(f"[SYNC] _call_llm_sync exception: {e}")
            raise

    def _generate_followup_questions_sync(self, context):
        try:
            prompt = self.followup_prompt.format(context=context)
            followup_completion = self.sync_llm_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": prompt}],
                temperature=0.7
            )
            ...
            print(f"[SYNC] _generate_followup_questions_sync exception: {e}")
            return []

    def generate_answer_stream_sync(self, question, top_k=3, user_history=None):
        print(f"[SYNC] generate_answer_stream_sync called with: {question}")
        try:
            if not self._filter_question_sync(question):
                print(f"[SYNC] Question filtered out (not smartstore)")
                yield {
                    "type": "final-error",
                    "answer": "스마트스토어 관련 질문만 답변할 수 있습니다.",
                    "similar_questions": [],
                ...
                return
            query_vec = self._embed_question_sync(question)
            results = self._search_similar_questions_sync(query_vec, top_k=top_k)
            if not results:
                print(f"[SYNC] No similar questions found.")
                yield {
                    "type": "final-error",
                    "answer": "적절한 답변을 찾지 못했습니다.",
                    "similar_questions": [],
                ...
                return
            faq_context = self._build_context_sync(results)
            if user_history:
                prompt = (
                    f"[이전 대화 이력]\n{user_history}\n\n"
                    f"[스마트스토어 FAQ/지식]\n{faq_context}\n\n"
            ...
            print(f"[SYNC] LLM answer prompt [stream]: {prompt}")
            stream = self.sync_llm_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": prompt}],
                temperature=0.7,
                stream=True
            ...
            followup_questions = self._generate_followup_questions_sync(faq_context)
            print(f"[SYNC] followup_questions [stream]: {followup_questions}")
            yield {
                "type": "final-success",
                "answer": "".join(collected_answer),
                "similar_questions": [r["question"] for r in results],
            ...
            print(f"[SYNC] generate_answer_stream_sync exception: {e}")
            yield {
                "type": "final-error",
                "answer": f"RAG 파이프라인 처리 중 오류가 발생했습니다: {e}",
                "similar_questions": [],
    ...
    # 기존 async 메서드는 그대로 두고, FastAPI 등에서는 async 버전 사용 가능